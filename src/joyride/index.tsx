...
export default function ControlledDemo() {
	...
	const handleJoyrideCallback = (data: CallBackProps) => {
		...
		if (([EVENTS.STEP_AFTER, EVENTS.TARGET_NOT_FOUND] as string[]).includes(type)) {
			...
			console.log(nextStepIndex, "=====CALLING");

			if (sidebarOpen && index === 0) {
				setState({ run: true });
			} else if (sidebarOpen && index === 1) {
				console.log("=======11111", nextStepIndex);
				setState({
					run: false,
					sidebarOpen: false,
				...
				console.log("======33333", nextStepIndex);
				setState({
					sidebarOpen: false,
					stepIndex: nextStepIndex,
				});
	...
	return (
		<div>
			...
			{/* </Menu> */}
			<div ref={menu} onClick={() => console.log("IN HER")}>
				hamburger
			</div>
			<div id="innerContainer">
				<h1>DASHBOARD</h1>
				<div>
					<div ref={calendar}>calendar</div>
					<div ref={growth}>growth</div>
					<div ref={users}>Users</div>
					<div ref={connections}>Connections</div>
				</div>
			</div>
		</div>
	);
